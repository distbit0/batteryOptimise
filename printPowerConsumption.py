...
def get_charge_direction(history):
    """Determine charging direction from history (-1 for discharging, 1 for charging, 0 if unclear)"""
    if len(history) < 2:
        return 0

    last_charge = history[-1][1]
    previous_charge = history[-2][1]
    if last_charge > previous_charge:
        return 1
    elif last_charge < previous_charge:
        return -1
    return 0
...
try:
    try:
        ...
        power_consumption = (
            float(read_file("/sys/class/power_supply/BAT*/power_now")) / 10**6
        )
except:
...
# Read battery's current charge and total capacity
try:
...
try:
    charge_control_end_threshold = float(
        read_file("/sys/class/power_supply/BAT*/charge_control_end_threshold")
    )
except:
    charge_control_end_threshold = 100

total_capacity = charge_control_end_threshold * charge_full / 100
...
log_file = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "charge_history.log"
)
history = load_charge_history(log_file)

# Determine current charging direction based on AC status
current_direction = 1 if ac_status == "1" else -1

# Check if there's history and if the direction has changed
if history:
    historical_direction = get_charge_direction(history)
    if historical_direction != 0 and historical_direction != current_direction:
        # Clear history if direction changed
        history = []
...
if avg_power_consumption != 0:
    avg_current = (
        avg_power_consumption * 10**12
    ) / voltage_now  # Convert back to microamps
    if avg_power_consumption > 0:
...
print(
    f"| {battery_percent}% | NOW: {power_consumption_rounded}W || AVG: {avg_power_consumption_rounded}W | {time_remaining_rounded}H |"
)
